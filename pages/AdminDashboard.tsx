...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentView }) => {
    ...
    const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean, title: string, message: string, type: 'danger' | 'warning' | 'default' | 'success', onConfirm: () => void }>({
        isOpen: false, title: '', message: '', type: 'default', onConfirm: () => { }
    });
    ...
    const openConfirm = (title: string, message: string, onConfirm: () => void, type: 'danger' | 'warning' = 'warning') => {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setToast({
                ...
                message: error instanceof Error ? error.message : 'No se pudo conectar con el servidor. Verifica tu conexión.'
            });
        }
    };
    ...
    const handleDeleteUser = (id: string) => {
        openConfirm('Eliminar Usuario', '¿Seguro que desea eliminar este usuario? Esta acción no se puede deshacer.', async () => {
    ...
    const handleDeleteCareer = (id: string) => {
        openConfirm('Eliminar Carrera', '¿Eliminar carrera? Se borrarán las asignaturas asociadas.', async () => {
    ...
    const handleDeleteCourse = (id: string) => {
        openConfirm('Eliminar Asignatura', '¿Eliminar asignatura? Esto afectará el historial de notas si existen registros.', async () => {
    ...
    const handleEnrollStudent = async () => {
        ...
        // Offer download via modal instead of window.confirm
        setConfirmModal({
            isOpen: true,
            title: 'Comprobante',
            message: 'Matrícula exitosa. ¿Desea descargar el comprobante de inscripción ahora?',
            type: 'success',
            onConfirm: async () => {
    ...
    const handleDeleteCampus = (id: string) => {
        openConfirm('Eliminar Recinto', '¿Está seguro de eliminar este recinto? Esta acción borrará TODOS los datos asociados (Usuarios, Cursos, Notas).', async () => {
    ...
    const updateScholarshipStatus = async (id: string, status: string) => {
        ...
        setToast({
            ...
            title: status === 'approved' ? 'Beca Aprobada' : 'Beca Rechazada',
            message: 'El estado de la solicitud ha cambiado.'
        });
        loadData();
    };

    const deleteScholarshipTypeAction = (id: string) => {
        openConfirm('Eliminar Beca', '¿Desea eliminar este tipo de beca? Esto no afectará las solicitudes ya aprobadas.', async () => {
            await api.deleteScholarshipType(id);
            setToast({ type: 'success', title: 'Beca Eliminada', message: 'Configuración actualizada.' });
            loadData();
            setConfirmModal({ ...confirmModal, isOpen: false });
        }, 'danger');
    };
        ...
        <div className="space-y-8 relative">
                        ...
                        <div className="space-y-3">
                            {scholarshipApps.map(app => (
                                <div key={app.id} className="p-4 border border-gray-100 rounded-xl flex justify-between items-center">
                                    ...
                                    {app.status === 'pending' ? (
                                        <div className="flex gap-2">
                                            <button onClick={() => updateScholarshipStatus(app.id, 'approved')} className="px-3 py-1 bg-green-50 text-green-700 rounded text-xs font-bold hover:bg-green-100">Aprobar</button>
                                            <button onClick={() => updateScholarshipStatus(app.id, 'rejected')} className="px-3 py-1 bg-red-50 text-red-700 rounded text-xs font-bold hover:bg-red-100">Rechazar</button>
                                        </div>
                            ...
                            {scholarshipTypes.map(t => (
                                <div key={t.id} className="flex justify-between items-center p-3 border border-gray-200 rounded-xl bg-gray-50">
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        <button onClick={() => deleteScholarshipTypeAction(t.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-lg"><Trash2 size={16} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {editScholarship && (
                <Modal isOpen={true} onClose={() => setEditScholarship(null)} title="Editar Beca">
                    <form onSubmit={(e) => {
                        e.preventDefault();
                        api.updateScholarshipType(editScholarship.id, editScholarship).then(() => {
                            setToast({ type: 'success', title: 'Beca Actualizada', message: 'Los cambios se han guardado.' });
                            loadData();
                        });
            ...
            <Modal
                ...
                footer={
                    <>
                        <button onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })} className="px-4 py-2 text-gray-600 font-medium hover:bg-gray-100 rounded-lg">Cancelar</button>
                        <button onClick={confirmModal.onConfirm} className={`px-4 py-2 text-white font-bold rounded-lg shadow-lg transition-colors ${confirmModal.type === 'danger' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}>Confirmar</button>
                    </>
                }
            >
